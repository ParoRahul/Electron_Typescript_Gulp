...
import { IServiceIdentifier, IInstantiationService, IServicesAccessor, _util, optional } from './instantiation'
import { ServiceCollection } from './serviceCollection'
import { IdleValue } from '../async'
import { illegalState } from '../errors'
...
export class InstantiationService implements IInstantiationService {
	...
	invokeFunction<R, TS extends any[] = []>(fn: (accessor: IServicesAccessor, ...args: TS) => R, ...args: TS): R {
		...
		try {
				const accessor: IServicesAccessor = {
					get: <T>(id: IServiceIdentifier<T>, isOptional?: typeof optional) => {
						if (_done) {
	...
	private _setServiceInstance<T>(id: IServiceIdentifier<T>, instance: T): void {
	...
	private _getServiceInstanceOrDescriptor<T>(id: IServiceIdentifier<T>): T | SyncDescriptor<T> {
	...
	private _getOrCreateServiceInstance<T>(id: IServiceIdentifier<T>, _trace: Trace): T {
	...
	private _createAndCacheServiceInstance<T>(id: IServiceIdentifier<T>, desc: SyncDescriptor<T>, _trace: Trace): T {
		type Triple = { id: IServiceIdentifier<any>, desc: SyncDescriptor<any>, _trace: Trace };
	...
	private _createServiceInstanceWithOwner<T>(id: IServiceIdentifier<T>, ctor: any, args: any[] = [], supportsDelayedInstantiation: boolean, _trace: Trace): T {
		if (this._services.get(id) instanceof SyncDescriptor) {
...
class Trace {
	...
	private readonly _dep: [IServiceIdentifier<any>, boolean, Trace?][] = [];
	...
	branch(id: IServiceIdentifier<any>, first: boolean): Trace {
