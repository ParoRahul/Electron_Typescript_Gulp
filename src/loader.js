 ...
 *---------------------------------------------------------------------------------------------
 *--------------------------------------------------------------------------------------------*/
let _amdLoaderGlobal = this
let _commonjsGlobal = typeof global === 'object' ? global : {}
var AMDLoader;
(function (AMDLoader) {
...
var AMDLoader;
(function (AMDLoader) {
    let LoaderEvent = /** @class */ (function () {
    ...
    let LoaderEventRecorder = /** @class */ (function () {
    ...
    let NullLoaderEventRecorder = /** @class */ (function () {
...
(function (AMDLoader) {
    function ensureError(err) {
        ...
        let result = new Error(err.message || String(err) || 'Unknown Error')
        if (err.stack) {
    ...
    let ConfigurationOptionsUtil = /** @class */ (function () {
        ...
        ConfigurationOptionsUtil.validateConfigurationOptions = function (options) {
            ...
            if (options.nodeCachedData && typeof options.nodeCachedData === 'object') {
                ...
                if (!options.nodeCachedData.path || typeof options.nodeCachedData.path !== 'string') {
                    let err = ensureError(new Error('INVALID cached data configuration, \'path\' MUST be set'))
    ...
    let Configuration = /** @class */ (function () {
        ...
        Configuration.prototype.moduleIdToPaths = function (moduleId) {
            let isNodeModule = ((this.nodeModulesMap[moduleId] === true)
...
(function (AMDLoader) {
     ...
     */
    let OnlyOnceScriptLoader = /** @class */ (function () {
        ...
        OnlyOnceScriptLoader.prototype.load = function (moduleManager, scriptSrc, callback, errorback) {
            ...
            if (!this._scriptLoader) {
                if (this._env.isWebWorker) {
                    ...
                    let preferScriptTags = moduleManager.getConfig().getOptionsLiteral().preferScriptTags
                    if (preferScriptTags) {
    ...
    var BrowserScriptLoader = /** @class */ (function () {
        ...
        BrowserScriptLoader.prototype.load = function (moduleManager, scriptSrc, callback, errorback) {
            if (/^node\|/.test(scriptSrc)) {
                ...
                let trustedTypesPolicy = moduleManager.getConfig().getOptionsLiteral().trustedTypesPolicy
                if (trustedTypesPolicy) {
                    scriptSrc = trustedTypesPolicy.createScriptURL(scriptSrc)
                }
                script.setAttribute('src', scriptSrc)
                // Propagate CSP nonce to dynamically created script tag.
                let cspNonce = moduleManager.getConfig().getOptionsLiteral().cspNonce
                if (cspNonce) {
    ...
    var NodeScriptLoader = /** @class */ (function () {
        ...
        NodeScriptLoader.prototype._initNodeRequire = function (nodeRequire, moduleManager) {
            ...
            let nodeCachedData = moduleManager.getConfig().getOptionsLiteral().nodeCachedData
            if (!nodeCachedData) {
                return
            }
            ...
            Module.prototype._compile = function (content, filename) {
                ...
                let scriptSource = Module.wrap(content.replace(/^#!.*/, ''))
                // create script
                let recorder = moduleManager.getRecorder()
                let cachedDataPath = that._getCachedDataPath(nodeCachedData, filename)
                let options = { filename: filename }
                let hashData
                try {
                    let data = that._fs.readFileSync(cachedDataPath)
                ...
                let script = new that._vm.Script(scriptSource, options)
                let compileWrapper = script.runInThisContext(options)
                // run script
                let dirname = that._path.dirname(filename)
                let require = makeRequireFunction(this)
                let args = [this.exports, require, this, filename, dirname, process, _commonjsGlobal, Buffer]
                let result = compileWrapper.apply(this.exports, args)
        ...
        NodeScriptLoader.prototype.load = function (moduleManager, scriptSrc, callback, errorback) {
            ...
            let opts = moduleManager.getConfig().getOptionsLiteral()
            let nodeRequire = ensureRecordedNodeRequire(moduleManager.getRecorder(), (opts.nodeRequire || AMDLoader.global.nodeRequire))
            let nodeInstrumenter = (opts.nodeInstrumenter || function (c) { return c })
            this._init(nodeRequire)
            this._initNodeRequire(nodeRequire, moduleManager)
            let recorder = moduleManager.getRecorder()
            if (/^node\|/.test(scriptSrc)) {
                let pieces = scriptSrc.split('|')
                let moduleExports_2 = null
                try {
                ...
                let normalizedScriptSrc_1 = this._path.normalize(scriptSrc)
                let vmScriptPathOrUri_1 = this._getElectronRendererScriptPathOrUri(normalizedScriptSrc_1)
                let wantsCachedData_1 = Boolean(opts.nodeCachedData)
                let cachedDataPath_1 = wantsCachedData_1 ? this._getCachedDataPath(opts.nodeCachedData, scriptSrc) : undefined
                this._readSourceAndCachedData(normalizedScriptSrc_1, cachedDataPath_1, recorder, function (err, data, cachedData, hashData) {
                    ...
                    let scriptOpts = { filename: vmScriptPathOrUri_1, cachedData: cachedData }
                    let script = _this._createAndEvalScript(moduleManager, scriptSource, scriptOpts, callback, errorback)
        ...
        NodeScriptLoader.prototype._createAndEvalScript = function (moduleManager, contents, options, callback, errorback) {
            ...
            let script = new this._vm.Script(contents, options)
            let ret = script.runInThisContext(options)
            let globalDefineFunc = moduleManager.getGlobalAMDDefineFunc()
            let receivedDefineCall = false
            let localDefineFunc = function () {
        ...
        NodeScriptLoader.prototype._getCachedDataPath = function (config, filename) {
            let hash = this._crypto.createHash('md5').update(filename, 'utf8').update(config.seed, 'utf8').digest('hex')
            let basename = this._path.basename(filename).replace(/\.js$/, '')
        ...
        NodeScriptLoader.prototype._readSourceAndCachedData = function (sourcePath, cachedDataPath, recorder, callback) {
            if (!cachedDataPath) {
                ...
                let source_1 = undefined
                let cachedData_1 = undefined
                let hashData_1 = undefined
                let steps_1 = 2
                let step_1 = function (err) {
                    if (err) {
                        callback(err)
                    }
                    else if (--steps_1 === 0) {
        ...
        NodeScriptLoader.prototype._verifyCachedData = function (script, scriptSource, cachedDataPath, hashData, moduleManager) {
            ...
            setTimeout(function () {
                ...
                let hashDataNow = _this._crypto.createHash('md5').update(scriptSource, 'utf8').digest()
    ...
    function ensureRecordedNodeRequire(recorder, _nodeRequire) {
        ...
        let nodeRequire = function nodeRequire(what) {
...
(function (AMDLoader) {
    // ------------------------------------------------------------------------
    // ModuleIdResolver
    let ModuleIdResolver = /** @class */ (function () {
    ...
    let ModuleIdProvider = /** @class */ (function () {
    ...
    let RegularDependency = /** @class */ (function () {
    ...
    let PluginDependency = /** @class */ (function () {
    ...
    let ModuleManager = /** @class */ (function () {
        ...
        ModuleManager._findRelevantLocationInStack = function (needle, stack) {
            let normalize = function (str) { return str.replace(/\\/g, '/') }
        ...
        ModuleManager.prototype.getBuildInfo = function () {
            ...
            for (let i = 0, len = this._modules2.length; i < len; i++) {
                ...
                let location_1 = this._buildInfoPath[m.id] || null
        ...
        ModuleManager.prototype._createLoadError = function (moduleId, _err) {
            ...
            let neededBy = (this._inverseDependencies2[moduleId] || []).map(function (intModuleId) { return _this._moduleIdProvider.getStrModuleId(intModuleId) })
            let err = AMDLoader.ensureError(_err)
            err.phase = 'loading'
        ...
        ModuleManager.prototype._createRequire = function (moduleIdResolver) {
            let _this = this
            let result = (function (dependencies, callback, errorback) {
        ...
        ModuleManager.prototype._loadModule = function (moduleId) {
            ...
            var loadNextPath = function (err) {
                ...
                if (lastPathIndex >= paths.length) {
                    ...
                    let currentPath_1 = paths[lastPathIndex]
                    let recorder_1 = _this.getRecorder()
...
var AMDLoader;
(function (AMDLoader) {
    let env = new AMDLoader.Environment()
    let moduleManager = null
    let DefineFunc = function (id, dependencies, callback) {
        if (typeof id !== 'string') {
    ...
    let _requireFunc_config = function (params, shouldOverwrite) {
        if (shouldOverwrite === void 0) { shouldOverwrite = false }
    ...
    function init() {
        if (typeof AMDLoader.global.require !== 'undefined' || typeof require !== 'undefined') {
            let _nodeRequire = (AMDLoader.global.require || require)
            if (typeof _nodeRequire === 'function' && typeof _nodeRequire.resolve === 'function') {
                // re-expose node's require function
                let nodeRequire = AMDLoader.ensureRecordedNodeRequire(moduleManager.getRecorder(), _nodeRequire)
